...
object HttpUtil {
     ...
     *
     * ВАЖНО: панели выбирают по нему формат выдачи, и это объявление формата, а не имя
     * программы - ровно как у браузеров, где «Mozilla/5.0» давно никого не обозначает.
     * Незнакомому клиенту панель отдаёт обычные ссылки, знакомому - JSON, а в нём
     * приходят и описания серверов (meta.serverDescription).
     *
     * Токен в начале строки панели и разбирают; всё, что дальше в скобках, свободно -
     * туда и вписано настоящее имя. Плюс приложение называет себя отдельным заголовком,
     * см. [clientUserAgent]: по нему панель показывает устройство в своём списке.
     */
    fun defaultUserAgent(): String = "INCY/1.0 (Ward/${BuildConfig.VERSION_NAME})"

    /**
     * Прежний User-Agent - на случай, если панель не поймёт нынешний.
     *
     * Панелей много, и какая-то может знать токен из [defaultUserAgent] и отдавать под
     * него формат, которого мы не понимаем. Тогда обновление повторяется с этой
     * строкой: её понимают все, а отдают под неё в худшем случае обычные ссылки.
     */
    fun legacyUserAgent(): String = "v2rayNG/${BuildConfig.VERSION_NAME}"
